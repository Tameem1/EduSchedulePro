...
export async function notifyManagerAboutCompletedAppointment(
  appointmentId: number,
  teacherId: number,
): Promise<boolean> {
  try {
    ...
    for (const manager of managers) {
      if (manager.telegramUsername) {
        ...
        if (sent) {
          anyNotificationSent = true;
          console.log(`Completion notification sent to manager ${manager.username}`);
        }
      }
    }
    ...
    console.error(
      "Failed to notify managers about completed appointment:",
      error instanceof Error ? error.message : String(error),
    );
    return false;
  }
